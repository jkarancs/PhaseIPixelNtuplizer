...
class PhaseIPixelNtuplizer : public edm::EDAnalyzer
{
	...
	private:
		...
		void                                getDigiData(const edm::Handle<edm::DetSetVector<PixelDigi>>& digiCollectionHandle, const std::map<uint32_t, int>& federrors);
#endif
		void                                getClustData(const edm::Handle<edmNew::DetSetVector<SiPixelCluster>>& clusterCollectionHandle, const std::map<uint32_t, int>& federrors);
		void                                getTrajTrackData(const edm::Handle<reco::VertexCollection>& vertexCollectionHandle, const edm::Handle<TrajTrackAssociationCollection>& trajTrackCollectionHandle, const std::map<uint32_t, int>& federrors);
		std::map<reco::TrackRef, TrackData> getTrackData(const edm::Handle<reco::VertexCollection>& vertexCollectionHandle, const edm::Handle<TrajTrackAssociationCollection>& trajTrackCollectionHandle, const std::map<uint32_t, int>& federrors);
	...
	private:
		void  propagateTrackToLayer1(const edm::Ref<std::vector<Trajectory>>& trajectory, const reco::TrackRef track, const std::map<uint32_t, int>& federrors);
		std::tuple<std::vector<TrajectoryMeasurement>::const_iterator, float> 
			findMatchingTrajMeasurement(const GlobalPoint& referencePoint, const ModuleData& referenceModInfo, const std::vector<TrajectoryMeasurement>& possibleMatches, const std::map<uint32_t, int>& federrors);
		...
		void       printTrackCompositionInfo(const edm::Ref<std::vector<Trajectory>>& trajectory, const reco::TrackRef& track, const edm::Handle<edmNew::DetSetVector<SiPixelCluster>> clusterCollectionHandle, const edm::Handle<reco::VertexCollection>& vertexCollectionHandle, const std::map<uint32_t, int>& federrors);
};
