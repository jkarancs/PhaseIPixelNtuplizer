...
void Occup_from_ntuple(std::string filename = "Ntuple*.root", int phase=1, bool save_plot=0) {
  ...
  for (int itree=0; itree<3; ++itree) if (digi_clu_hit[itree]&&!(phase==1&&itree==0)) {
    ...
    tree->Add((filename+"/"+trees[itree]).c_str());
	...
	h2 = new TH2D( (varname[itree]+"_Ring1").c_str(),
		       (std::string(";ROC / Disk;ROC / Blade;Ring 1 - ")+zaxis[itree]+" Occupancy").c_str(),
		       56*divide_roc,-3.5,3.5, 92*divide_roc,-11.5,11.5);
      }
      if      (phase==0) {
	...
	//dress_occup_plot(h2, 0, phase);
	c = custom_can_(h2, (std::string("Ring1/")+varname[itree]+"_vs_ROCBlade_vs_ROCDisk").c_str(),0,0,800,800,80,140);
	tree->Draw((std::string("mod_on.blade_panel_coord-0.5*mod_on.blade:mod_on.disk_coord>>")+h2->GetName()).c_str(),"det==1&&ring==1","COLZ");
	dress_occup_plot(h2, -1, phase);
	if (save_plot) save_plots(c, save_dir, formats, remove_prelim_label);
	TH2D* h3 = new TH2D( (varname[itree]+"_Ring2").c_str(),
		       (std::string(";ROC / Disk;ROC / Blade;Ring 2 - ")+zaxis[itree]+" Occupancy").c_str(),
		       56*divide_roc,-3.5,3.5, 140*divide_roc,-17.5,17.5);
	c = custom_can_(h3, (std::string("Ring2/")+varname[itree]+"_vs_ROCBlade_vs_ROCDisk").c_str(),0,0,800,800,80,140);
	tree->Draw((std::string("mod_on.blade_panel_coord:mod_on.disk_coord>>")+h3->GetName()).c_str(),"det==1&&ring==2","COLZ");
	dress_occup_plot(h3, -2, phase);
      }
      if (save_plot) save_plots(c, save_dir, formats, remove_prelim_label);
    }
  }
