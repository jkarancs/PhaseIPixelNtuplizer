...
void era_lat_(double xmin, double xmax, double ymin, double ymax) {
  std::string prelim = "#sqrt{s}=13 TeV";// #font[22]{Times bold} and #font[12]{Times Italic}
...
void save_plots(TCanvas* c, std::string save_dir, std::vector<std::string> formats, int remove_prelim_label=1) {  
  ...
  if (remove_prelim_label==2) {
    for (int i=0; i<c->GetListOfPrimitives()->GetEntries(); ++i) {
      std::string title = c->GetListOfPrimitives()->At(i)->GetTitle();
      size_t f = title.find("Preliminary");
      if (f!=std::string::npos) ((TLatex*)c->GetListOfPrimitives()->At(i))->SetTitle(title.replace(f,11,"Work in progress").c_str());
    }
  }
  gPad->Update(); 
...
void remove_plot(TCanvas* c, int iplot) {
  TLegend *leg = 0;
  ...
  if (leg) leg->GetListOfPrimitives()->RemoveAt(iplot+1);
  c->GetListOfPrimitives()->RemoveAt(iplot);
}

void remove_plot(TCanvas* c, int iplot, int ileg, bool ncol1=0) {
  TLegend *leg = 0;
  for (int i=0, n=c->GetListOfPrimitives()->GetEntries(); i<n; ++i) {
    if (std::string(c->GetListOfPrimitives()->At(i)->GetName())=="TPave") 
      leg = (TLegend*)c->GetListOfPrimitives()->At(i);
  }
  if (leg) leg->GetListOfPrimitives()->RemoveAt(ileg);
  if (iplot>0) c->GetListOfPrimitives()->RemoveAt(iplot);
  if (ncol1) leg->SetNColumns(1);
}


void modify_legend(TCanvas* c, std::string title, double x1, double x2, double y1, double y2) {
  TLegend *leg = 0;
  for (int i=0, n=c->GetListOfPrimitives()->GetEntries(); i<n; ++i) {
    if (std::string(c->GetListOfPrimitives()->At(i)->GetName())=="TPave") 
      leg = (TLegend*)c->GetListOfPrimitives()->At(i);
  }
  if (leg) leg->SetHeader(title.c_str()); leg->SetX1(x1); leg->SetX2(x2); leg->SetY1(y1); leg->SetY2(y2);
}

void rename_legend(TCanvas* c, std::string title) {
  TLegend *leg = 0;
  for (int i=0, n=c->GetListOfPrimitives()->GetEntries(); i<n; ++i) {
    if (std::string(c->GetListOfPrimitives()->At(i)->GetName())=="TPave") 
      leg = (TLegend*)c->GetListOfPrimitives()->At(i);
  }
  if (leg) leg->SetHeader(title.c_str());
}

void move_legend(TCanvas* c, double x1, double y2) {
TLegend *leg = 0;
  for (int i=0, n=c->GetListOfPrimitives()->GetEntries(); i<n; ++i) {
    if (std::string(c->GetListOfPrimitives()->At(i)->GetName())=="TPave") 
      leg = (TLegend*)c->GetListOfPrimitives()->At(i);
...
void add_years(TCanvas* c, double yearLabelYPos, double slope = 0) {
  TH1D *h = 0;
  for (int i=0, n=c->GetListOfPrimitives()->GetEntries(); i<n; ++i) if (h==0) {
    if (TString(c->GetListOfPrimitives()->At(i)->GetName()).Contains("_vs_IntLumi")) h = (TH1D*)c->GetListOfPrimitives()->At(i);
  }
  ...
  const double yearLimitX[3]    = {    0.5*(6.03+6.43),0.5*(29.40+30.53),0.5*(34.13+34.28)};
  const double yearLabelXPos[4] = {0.5*yearLimitX[0],0.5*(yearLimitX[0]+yearLimitX[1]),0.5*(yearLimitX[1]+yearLimitX[2]),55};;
  TLine *line[3];
  for (int i=0; i<3;++i){
    line[i] = new TLine (yearLimitX[i],ymin, yearLimitX[i], ymax);
  ...
  TText  *t[4];  
  t[0] = new TText(yearLabelXPos[0],yearLabelYPos+yearLabelXPos[0]*slope,"2011");
  t[1] = new TText(yearLabelXPos[1],yearLabelYPos+yearLabelXPos[1]*slope,"2012");
  t[2] = new TText(yearLabelXPos[2],yearLabelYPos+yearLabelXPos[2]*slope,"2015");
  t[3] = new TText(yearLabelXPos[3],yearLabelYPos+yearLabelXPos[3]*slope,"2016");
  for (int i=0;i<4;++i){
    t[i]->SetTextAlign(22);
    t[i]->SetTextColor(kGray+4);
    t[i]->SetTextFont(62);
    t[i]->SetTextSize(0.04);
    t[i]->SetTextAngle(i%2?0:90);
    t[i]->Draw();
  }
}

void add_year_Run2(TCanvas* c, double yearLabelYPos, double slope = 0) {
  TH1D *h = 0;
  for (int i=0, n=c->GetListOfPrimitives()->GetEntries(); i<n; ++i) if (h==0) {
    if (TString(c->GetListOfPrimitives()->At(i)->GetName()).Contains("_vs_IntLumi")) h = (TH1D*)c->GetListOfPrimitives()->At(i);
  }
  ...
  double ymin = h->GetMinimum();
  double ymax = h->GetMaximum();
  const double yearLimitX =  0.5*(4.20+4.35);
  const double yearLabelXPos[2] = { 0.5*(0+yearLimitX),25};;
  TLine *line;
  line = new TLine (yearLimitX,ymin, yearLimitX, ymax);
  line->SetLineStyle(2);
  line->SetLineColor(1);
  line->SetLineWidth(1);  
  line->Draw();
  TText  *t[2];
  t[0] = new TText(yearLabelXPos[0],yearLabelYPos+yearLabelXPos[0]*slope,"2015");
  t[1] = new TText(yearLabelXPos[1],yearLabelYPos+yearLabelXPos[1]*slope,"2016");
  for (int i=0;i<2;++i){
    t[i]->SetTextAlign(22);
    t[i]->SetTextColor(kGray+4);
    t[i]->SetTextFont(62);
...
void fit_sigmoid_functions_(TCanvas* c, int first, int last) {
  for (int i=first; i<=last; ++i) {
    TGraphAsymmErrors *g = (TGraphAsymmErrors*)c->GetListOfPrimitives()->At(i);
    TF1* f = new TF1((std::string(g->GetName())+"_fit").c_str(), "[0]+[1]*x+[2]/(1+exp(([3]-x)/[4]))",
    ...
    g->Fit((std::string(g->GetName())+"_fit").c_str(),"RM");
  }
}

void draw_straight_lines_(TCanvas* c, int first, int last) {
  for (int i=first; i<=last; ++i) {
    TH1D *h = (TH1D*)c->GetListOfPrimitives()->At(i);
    double x1=0,x2=0,y1=0,y2=0;
    bool first = true;
    for (int i=1; i<=h->GetNbinsX(); ++i) if (h->GetBinContent(i)>0) {
      x1 = x2;
      y1 = y2;
      x2 = h->GetXaxis()->GetBinCenter(i);
      y2 = h->GetBinContent(i);
      if (!first) {
	TLine *l = new TLine(x1,y1,x2,y2);
	l->SetLineWidth(2);
...
void usual(std::string name, double xmin=-1, double xmax=-1, double ymin=-1, double ymax=-1, double leg_x1=-1, double leg_y2=-1) {
  if (!f) std::cout<<"No such file exist\n"<<std::endl;
	...
	if (leg_x1>=0&&leg_y2>=0) move_legend(c, leg_x1, leg_y2);
