					 ...
					 std::string draw = "COLZ", bool norm = false, bool log = false)
{
  ...
  if(norm && h->Integral() > 0)
	h = (TH2D *)h->DrawNormalized(draw.c_str());
  else
	h->Draw(draw.c_str());
...
void asym_labels_(TH1D *orig, TGraphAsymmErrors *tgae, int hor_vert_up_down = 1)
{
  ...
  for(size_t i = 0; i < (size_t)(orig->GetNbinsX()); ++i)
  {
							...
							Double_t left_localymax_x = NOVAL_F, Double_t shift_maximum = 1.0)
{
  if(hleft->GetEntries() > 0 && hleft->GetMaximum() != 0 && hright->GetEntries() > 0 && hright->GetMaximum() != 0)
  {
	...
	Double_t left_maximum  = left_localymax_x == NOVAL_F ? hleft->GetMaximum() : hleft->GetBinContent(hleft->GetXaxis()->FindBin(left_localymax_x));
...
void draw_line_old(double x1, double y1, double x2, double y2, Color_t col = 1, Style_t s = 1, Width_t w = 1)
{
  ...
  if(col != 1) line->SetLineColor(col);
  if(s != 1) line->SetLineStyle(s);
  line->SetLineWidth(w);
  line->Draw();
}
...
void prelim_lat_(double xmin, double xmax, double ymin, double ymax, bool in)
{
  ...
  if(in)
  {
	TLatex *cms_lat = new TLatex(xmin + (xmax - xmin) / 20.0, ymax - (ymax - ymin) / 10.0, prelim.c_str());
	cms_lat->SetLineWidth(2);
	cms_lat->Draw();
  }
  else
  {
	TLatex *cms_lat = new TLatex(xmin, ymax + (ymax - ymin) / 40.0, prelim.c_str());
	cms_lat->SetLineWidth(2);
	cms_lat->Draw();
  }
}
...
void era_lat_(double xmin, double xmax, double ymin, double ymax)
{
  ...
  TLatex *era_lat    = new TLatex(xmax, ymax + (ymax - ymin) / 25.0, prelim.c_str());
...
void add_mpv_fit(TCanvas *c, int nhist)
{
  ...
  for(int i = 0; i < nhist; ++i)
  {
	...
	f->Draw("SAME");
	std::stringstream ss;
	ss << "MPV " << std::setprecision(3) << mpv;
	TLatex *mpv_lat = new TLatex(xmax - (xmax - xmin) * 0.23, ymax - ((i + 0.25) * (ymax - ymin) / 20.0), ss.str().c_str());
	mpv_lat->SetTextAlign(13);
	mpv_lat->SetTextSize(0.04);
	mpv_lat->SetTextFont(42);
	mpv_lat->SetLineWidth(2);
	mpv_lat->SetTextColor(h->GetLineColor());
	mpv_lat->Draw("SAME");
  }
}
...
void add_mean(TCanvas *c, int nhist)
{
  ...
  for(int i = 0; i < nhist; ++i)
  {
	...
	mean_lat->SetTextAlign(13);
	mean_lat->SetTextSize(0.04);
	mean_lat->SetTextFont(42);
	mean_lat->SetLineWidth(2);
	mean_lat->SetTextColor(h->GetLineColor());
	mean_lat->Draw("SAME");
  }
}
...
{
  return fit_turnon_improved_((TH1D *)(h->ProfileX()), (TH1D *)(h->ProjectionX()), draw);
}
...
double vturnon_(TH1D *h)
{
  ...
  if(f != 0)
	return f->GetX(f->GetMaximum() - 0.01);
  else
	return -9999;
}
...
void add_turnon_fits_(TCanvas *c, std::vector<TH2D *> parents, double xmin = -2.5, double xmax = 202.5, double ymin = 0, double ymax = 1)
{
  ...
  for(size_t i = 0; i < parents.size(); ++i)
  {
	...
	gPad->Update();
	std::stringstream ss;
	ss << std::setprecision(3) << vturnon_(fit);
...
{
  if(!f)
	std::cout << "No such file exist\n"
			  << std::endl;
  else
  {
	c = (TCanvas *)f->Get(name.c_str());
	if(!c)
	  std::cout << "Canvas doesn't exist: " << name << std::endl;
	else
	{
	  c = (TCanvas *)c->Clone();
	  c->Draw();
			 ...
			 float x1 = -1, float x2 = -1, float y1 = -1, float y2 = -1)
{
  ...
  else
  {
	...
	else
	{
	  ...
	  if(save)
		c->SaveAs((out + "." + ext).c_str());
	  else
		std::cout << "Showing plot: " << canname << std::endl;
	  f->Close();
	}
  }
}
...
{
  // Get Profile Histo
  TH1D *normx = nullptr, *normy = nullptr;
  ...
  if(along_x)
  {
	...
	for(int binx = 1; binx <= h->GetNbinsX(); ++binx)
	  if(normx->GetBinContent(binx) > 0) countx++;
	double avgx = normx->Integral() / countx;
	for(int binx = 1; binx <= h->GetNbinsX(); ++binx)
	  if(normx->GetBinContent(binx) > 0)
  ...
  if(along_y)
  {
	...
	for(int biny = 1; biny <= h->GetNbinsY(); ++biny)
	  if(normy->GetBinContent(biny) > 0) county++;
	double avgy = normy->Integral() / county;
	for(int biny = 1; biny <= h->GetNbinsY(); ++biny)
	  if(normy->GetBinContent(biny) > 0)
